...
class ContentPane (gtk.Notebook):
    ...
    def _progress_cb(self, view, progress):
        #print ('PROGRESS: ' + str(view.get_progress()))
        if view.get_progress() >= 1:
            global murl
            print('PAGING LOADING TIME: ' + murl + ':' + str(self.finishedCnt) + ":" + str(time.time() - self.time))
    ...
    def _construct_tab_view (self, web_view, url=None):
        ...
        web_view.connect('notify::progress', self._progress_cb)
...
#and replace with a new persistent jar
        cookiejar = libsoup.soup_cookie_jar_text_new('cookies.txt',False)
        libsoup.soup_session_add_feature(session, cookiejar)

        global murl
        murl = sys.argv[1]
        content_tabs.new_tab(sys.argv[1])
