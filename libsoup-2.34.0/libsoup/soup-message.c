...
soup_message_class_init (SoupMessageClass *message_class)
{
	...
	g_object_class_install_property (
		object_class, PROP_URI,
		g_param_spec_boxed (SOUP_MESSAGE_FAKE_URI,
				    "URI",
				    "The message's Request-Fake-URI",
				    SOUP_TYPE_URI,
				    G_PARAM_READWRITE));
...
void
soup_message_set_fake_uri (SoupMessage *msg, SoupURI *uri)
{

	SoupMessagePrivate *priv;

	fprintf(stderr, "[[[[[[[[[[[[[[[[[set_fake_uri is called in libsoup\n"); fflush(stderr);

	g_return_if_fail (SOUP_IS_MESSAGE (msg));
	priv = SOUP_MESSAGE_GET_PRIVATE (msg);

	if (priv->fake_uri)
		soup_uri_free (priv->fake_uri);
	if (priv->addr) {
		g_object_unref (priv->addr);
		priv->addr = NULL;
	}

	//priv->fake_uri = soup_uri_copy (uri);
	priv->uri = soup_uri_copy (uri);
	//g_object_notify (G_OBJECT (msg), SOUP_MESSAGE_FAKE_URI);
	g_object_notify (G_OBJECT (msg), SOUP_MESSAGE_URI);
}
...
SoupURI *
soup_message_get_fake_uri (SoupMessage *msg)
{
	g_return_val_if_fail (SOUP_IS_MESSAGE (msg), NULL);

	return SOUP_MESSAGE_GET_PRIVATE (msg)->fake_uri;
}
 ...
 * URI on a message, this will be unresolved, although the message's
 o* session will resolve it before sending the message.
 *
