...
static SoupCookie *
parse_one_cookie (const char *header, SoupURI *origin, gboolean isKCookie)
{
	const char *start, *end, *p;
	gboolean has_value;
	SoupCookie *cookie;	
...
#define MATCH_NAME(name) ((end - start == strlen (name)) && !g_ascii_strncasecmp (start, name, end - start))
	...
	if (cookie->domain) {
		 ...
		 */	    
		/* DON: do this only if this is not a cookie string
		   from the kernel */
		/* because we have to use the domain as itself for the domain if this cookie string is from the kernel */
		if (!g_hostname_is_ip_address (cookie->domain) &&
		    cookie->domain[0] != '.' && isKCookie == FALSE) {
...
{
	return parse_one_cookie (cookie, origin, FALSE);
}

SoupCookie *
soup_kcookie_parse (const char *cookie)
{
	return parse_one_cookie (cookie, NULL, TRUE);
}
...
soup_cookies_from_response (SoupMessage *msg)
{
	...
	while (soup_message_headers_iter_next (&iter, &name, &value)) {
		...
		cookie = parse_one_cookie (value, origin, FALSE);
		if (cookie)
