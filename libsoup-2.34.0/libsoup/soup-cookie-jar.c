...
int soup_cookie_hash(char* request_param) {
	int hash = 0;
	while(*request_param != 0) {
		hash += *request_param;
		++request_param;
	}
	hash = hash % 1024;
...
soup_cookie_jar_get_cookies (SoupCookieJar *jar, SoupURI *uri,
			     gboolean for_http)
{
	static char* cookie_cache[1024] = {NULL};

	SoupCookieJarPrivate *priv;
	GSList *cookies, *domain_cookies;
	char *domain, *cur, *next_domain, *result, *result_kernel;
	...
	char* cookies_str_next = NULL;

	SoupCookie* soup_cookie = NULL;


	// update cookie jar with invalidated cookies
	if (soup_get_invalidated_cookies() != NULL) {
#ifdef PRINT_DBG_MSG
		fprintf(stderr, "soup_get_invalidated_cookies() is not NULL\n"); fflush(stderr);
#endif
		SoupCookieList* m_head = soup_get_invalidated_cookies();
		SoupCookieList* next = NULL;
		while(m_head) {
			next = m_head->next;
			if(!m_head->soup_cookie) {
#ifdef PRINT_DBG_MSG
				fprintf(stderr, "m_head->soup_cookie is NULL: skipped\n"); fflush(stderr);
#endif
			} else {
				soup_cookie_jar_add_cookie_with_flag (jar,soup_cookie_copy(m_head->soup_cookie), TRUE);
			}
			m_head = next;
		}
		soup_empty_invalidated_cookies();
	}

	/* Sorin */
	if (soup_get_kcookies_flag() && soup_get_t2k_raw_socket()) {
		/* fprintf(stderr, "\nCOOKIE: soup_cookie_jar_get_cookies: %s, %s, %s\n",uri->scheme, uri->host, uri->path); */
		cookie_key = g_strdup_printf("%s://%s%s", uri->scheme, uri->host, uri->path);
		cookie_hash = soup_cookie_hash(cookie_key);

#ifdef PRINT_DBG_MSG
		fprintf(stderr, "\nCOOKIE: cookie_hash:%d\n",cookie_hash);
		fflush(stderr);
#endif

		if (1 && ((cookie_cache[cookie_hash] != NULL && strcmp(cookie_cache[cookie_hash], cookie_key) == 0))) {
#ifdef PRINT_DBG_MSG
			fprintf(stderr, "\nCOOKIE: cache hit!\n");
			fflush(stderr);
#endif
		} else {
			if(cookie_cache[cookie_hash] != NULL) g_free(cookie_cache[cookie_hash]);
			cookie_cache[cookie_hash] = g_strdup(cookie_key);
#ifdef PRINT_DBG_MSG
			fprintf(stderr, "\nCOOKIE: cache miss!\n");
			fflush(stderr);
#endif
			result_kernel = get_cookies_from_kernel(uri, for_http);

#ifdef PRINT_DBG_MSG			
			fprintf(stderr, "\nCOOKIE: result_kernel:%s\n", result_kernel);
			fflush(stderr);
#endif

			cookies_str = result_kernel;
			cookies_str_next = NULL;
			...
			while((cookies_str_next = strstr(cookies_str, "@@@")) != NULL) {
				*cookies_str_next = 0;
				...
				soup_cookie = soup_kcookie_parse(cookies_str);
				/* if(soup_cookie != NULL) { */
				/* 	fprintf(stderr, "p cookie->name : %s\n", soup_cookie->name); */
				/* 	fprintf(stderr, "p cookie->value : %s\n", soup_cookie->value); */
				/* 	fprintf(stderr, "p cookie->domain : %s\n", soup_cookie->domain); */
				/* 	fprintf(stderr, "p cookie->path : %s\n", soup_cookie->path); */
				/* 	fprintf(stderr, "p cookie->secure : %d\n", soup_cookie->secure); */
				/* 	fprintf(stderr, "p cookie->http_only : %d\n", soup_cookie->http_only); */
				/* } else { */
				/* 	fprintf(stderr, "p cookie is null\n"); */
				/* } */
				/* fflush(stderr); */
				soup_cookie_jar_add_cookie(jar, soup_cookie);
				cookies_str = cookies_str_next + 3;
			}
	...
	if (cookies) {
#ifdef PRINT_DBG_MSG			
		fprintf(stderr, "there does exist cookies\n");
#endif
	...
	if (soup_get_kcookies_flag() && soup_get_t2k_raw_socket()) {
		if (result) {
#ifdef PRINT_DBG_MSG			
			fprintf(stderr, "\nCOOKIE: soup_cookie_jar_get_cookies result: %s\n", result);
			fflush(stderr);
#endif
		}


		//if (g_strcmp0(result_kernel, result)) {
		//	cmd = g_strdup_printf("./cookie-cmp.py '%s' '%s'", result, result_kernel);
		//	system(cmd);
		//	g_free(cmd);
		//} else {
		//	fprintf(stderr, "COOKIES MATCH\n");
		//}
		if(result) {
			g_free(result);
		}
...
#ifdef PRINT_DBG_MSG			
	fprintf(stderr, "result cookie :%s\n", result);
	fflush(stderr);
#endif
	return result;
}
...
soup_cookie_jar_add_cookie (SoupCookieJar *jar, SoupCookie *cookie) {
	soup_cookie_jar_add_cookie_with_flag (jar, cookie, FALSE);
}
 ...
 **/
void
soup_cookie_jar_add_cookie_with_flag (SoupCookieJar *jar, SoupCookie *cookie, gboolean isItInvalidationCookie)
{
	SoupCookieJarPrivate *priv;
	...
	/* Sorin */
	if (!isItInvalidationCookie && soup_get_kcookies_flag() && soup_get_t2k_raw_socket()) {
		send_cookie_to_kernel(cookie);
		/* fprintf(stderr, "\nCOOKIE ADD %s %s\n", */
					     ...
					     const char *cookie)
{
	...
	//fprintf(stderr, "set_cookie(first_party)\n");
	//fflush(stderr);

	if (soup_cookie) {
		/*
		fprintf(stderr, "s cookie->name : %s\n", soup_cookie->name);
		fprintf(stderr, "s cookie->value : %s\n", soup_cookie->value);
		fprintf(stderr, "s cookie->domain : %s\n", soup_cookie->domain);
		fprintf(stderr, "s cookie->path : %s\n", soup_cookie->path);
		fprintf(stderr, "s cookie->secure : %d\n", soup_cookie->secure);
		fprintf(stderr, "s cookie->http_only : %d\n", soup_cookie->http_only);
		fflush(stderr);
		*/
...
process_set_cookie_header (SoupMessage *msg, gpointer user_data)
{
	...
	for (nc = new_cookies; nc; nc = nc->next) {
		...
		if ((priv->accept_policy == SOUP_COOKIE_JAR_ACCEPT_NO_THIRD_PARTY &&
		    ...
		    priv->accept_policy == SOUP_COOKIE_JAR_ACCEPT_ALWAYS) {
			SoupCookie* soup_cookie = nc->data;
			/*
			fprintf(stderr, "h cookie->name : %s\n", soup_cookie->name);
			fprintf(stderr, "h cookie->value : %s\n", soup_cookie->value);
			fprintf(stderr, "h cookie->domain : %s\n", soup_cookie->domain);
			fprintf(stderr, "h cookie->path : %s\n", soup_cookie->path);
			fprintf(stderr, "h cookie->secure : %d\n", soup_cookie->secure);
			fprintf(stderr, "h cookie->http_only : %d\n", soup_cookie->http_only);
			fflush(stderr);
			*/
