...
class ContentPane (gtk.Notebook):
    ...
    def load (self, text):
        ...
        if string.find(text, "http") != 0:
            print "ERROR occured"

        self.frames[self.get_nth_page(self.get_current_page()).get_child().get_main_frame()] = self.get_origin(text)
        print "the main frame is loaded : " + self.get_origin(text)

        child = self.get_nth_page(self.get_current_page())
        view = child.get_child()
        f = open('./urlfile.list', 'w')
        f.write(text)
        f.close()
        view.open(text)
    ...
    def get_uri(self, origin, uri):
        subprocess.call(['./mywget.sh',origin, uri])
        f = open("webfile")
        content = f.read()
        f.close()
        return content
    ...
    def _navigation_cb(self, view, frame, request, action, policy):
        uri = request.get_uri()

        if string.find(uri, "file://") != 0:
            if frame in self.frames and self.frames[frame] == self.get_origin(uri) :
                return
            self.frames[frame] = self.get_origin(uri)
            self.frames_load_status[frame] = 0
            print "frame's origin is navigated to :" + self.get_origin(uri) + ",mainframe?=" + str(view.get_main_frame() == frame) + "," + uri + ",FrameURI:" + str(frame.get_uri())
    ...
    def _resource_cb(self, view, frame, resource, request, response):
        ...
        # first resource loading in this frame. even navigation callback wasn't called for this frame
        if not (frame in self.frames) :
            self.frames[frame] = self.get_origin(uri)
            self.frames_load_status[frame] = 0
            print "frame's origin is navigated(2) to :" + self.get_origin(uri) + ",mainframe?=" + str(view.get_main_frame() == frame) + "," + uri
            return
            
        # same origin. let the tab create a socket directly to the server
        if string.find(uri, self.frames[frame]) == 0:
            return
        
        if self.frames_load_status[frame] == 0:
            self.frames_load_status[frame] = 1
            self.frames[frame] = self.get_origin(uri)
            print "frame's origin is changed to :" + self.get_origin(uri) + ",mainframe?=" + str(view.get_main_frame() == frame) + "," + uri
            return
        
        if response != None and self.frames_load_status[frame] == 1:
            self.frames[frame] = self.get_origin(uri)
            print "frame's origin is changed to :" + self.get_origin(uri) + ",mainframe?=" + str(view.get_main_frame() == frame) + "," + uri
            return

        if response == None and self.frames_load_status[frame] == 1:
            self.frames_load_status[frame] = 2

        fname = frame.get_name()
        
        #print "X-DOMAIN:uri " + uri + " on " + self.frames[frame] + " real:" + frame.get_uri()
        content = self.get_uri(self.frames[frame] + "\n:frame.get_uri():" + str(frame.get_uri()) + "\n:mainframe?=" + str(frame == view.get_main_frame()) + "\n:framename="+ fname, uri)
...
class WebBrowser(gtk.Window):
    ...
    def __init__(self):
        ...
        content_tabs.new_tab("about:blank")
...
if __name__ == "__main__":
    # setting the socket file fd that the tab is using to communicate with the kernel
    # so that libsoup can use it to create the socket through the kernel later
    from ctypes import *
